package View;

public class LoginView {
    public static void printloginView() {
        System.out.println("로그인 모듈 입니다");
        System.out.println("1.로그인 2.회원가입  0.메인 모듈");
    }

    public static void printenterView1() {
        System.out.println("로그인 접속");

    }
    public static void printenterView2() {

        System.out.println("ID를 입력해주세요: ");

    }

    public static void printenterView3() {

        System.out.println("PW를 입력해주세요: ");
    }

    public static void printsignView1() {
        System.out.println("회원가입");
    }

    public static void printsignView2() {
        System.out.println("ID를 입력해주세요: ");
    }

    public static void printsignView3() {
        System.out.println("PW를 입력해주세요: ");
    }

    public static void printsignView4() {
        System.out.println("이름을 입력해주세요: ");
    }

}
